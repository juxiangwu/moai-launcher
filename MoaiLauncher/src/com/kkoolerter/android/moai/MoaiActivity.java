...
public class MoaiActivity extends Activity {
	private static final String TAG = "MoaiActivity";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mWorkDirectory = getIntent().getStringExtra(KEY_WORK_DIRECTORY);
		Log.d(TAG, "onCreate:mWorkDiectory = " + mWorkDirectory);
		...
		// ApplicationInfo myApp = getPackageManager ().getApplicationInfo (
		// getPackageName (), 0 );

		// Moai.mount ( "bundle", myApp.sourceDir );
		// String workDir =
		// Environment.getExternalStorageDirectory()+"/moai/test/lua";
		// File file = new File(workDir);
		// if(!file.exists()){
		// file.mkdirs();
		// }
		Moai.setWorkingDirectory(mWorkDirectory);
		// } catch ( NameNotFoundException e ) {

		// MoaiLog.e (
		// "MoaiActivity onCreate: Unable to locate the application bundle" );
		// }
		...
		Display display = ((WindowManager) getSystemService(Context.WINDOW_SERVICE))
				.getDefaultDisplay();
		ConfigurationInfo info = ((ActivityManager) getSystemService(Context.ACTIVITY_SERVICE))
				.getDeviceConfigurationInfo();

		mMoaiView = new MoaiView(this, display.getWidth(), display.getHeight(),
				info.reqGlEsVersion);
	...
	protected void onResume() {
		...
		if (mAccelerometerListener != null) {
			...
			mSensorManager.registerListener(mAccelerometerListener,
					mMagnetometerSensor, SensorManager.SENSOR_DELAY_NORMAL);
		}
	...
	private void enableAccelerometerEvents(boolean enabled) {
		...
		if (!enabled) {
			...
			if (mAccelerometerSensor == null) {
				...
				mMagnetometerSensor = mSensorManager
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		MoaiLog.i("MoaiActivity onKeyDown, keycode " + keyCode + " event: "
				+ event);
	...
	private class ConnectivityBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			MoaiLog.i("ConnectivityBroadcastReceiver onReceive: Connection = "
					+ connectionType);
	...
	private class AccelerometerEventListener implements SensorEventListener {
		...
		// function.
		public void canonicalOrientationToScreenOrientation(
				int displayRotation, float[] canVec, float[] screenVec) {

			final int axisSwap[][] = { { 1, -1, 1, 0 }, // ROTATION_0
					{ 1, 1, 0, 1 }, // ROTATION_90
		...
		public void onSensorChanged(SensorEvent event) {
			...
			if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
				...
				Display display = ((WindowManager) getSystemService(Context.WINDOW_SERVICE))
						.getDefaultDisplay();
				canonicalOrientationToScreenOrientation(display.getRotation(),
						event.values, mAccelerometerData);
			...
			if (mGravity != null
					&& mGeomagnetic != null
					&& SensorManager.getRotationMatrix(mRotationMatrixA, null,
							mGravity, mGeomagnetic)) {

				int deviceId = Moai.InputDevice.INPUT_DEVICE.ordinal();
				int sensorId = Moai.InputSensor.SENSOR_COMPASS.ordinal();

				SensorManager.remapCoordinateSystem(mRotationMatrixA,
	...
	private class LocationEventListener implements LocationListener {
		...
		public void onLocationChanged(Location location) {
			...
			Moai.enqueueLocationEvent(deviceId, sensorId, longitude, latitude,
					altitude, hAccuracy, vAccuracy, speed);
		}
